...
The subsequent forensic A1 continuation exposed a storage-design issue in the diagnostic tooling, not a model or protocol issue. The original warning checkpoint condition was level-triggered and could save an approximately 65 MB checkpoint on every warning batch while a threshold remained exceeded. The supplied archive contains 172 warning events (143 training and 29 validation), but no checkpoint files. The forensic tool was corrected so warning checkpoints are emitted only on threshold crossings within a warning episode; sustained warnings continue to be logged without repeated checkpoint writes, and a new checkpoint is allowed only after the condition clears and recrosses. Failure checkpoints remain retained, and regular forensic checkpoints default to a 10-epoch interval. This retention policy applies only to diagnostic tooling and does not change the approved A1 experiment protocol. Existing A1 history was preserved, no full forensic run was rerun, and the original epoch-27 failure remains unlocalized.
